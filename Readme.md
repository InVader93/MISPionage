...
## Overview MISPionage is a Python script designed to automate the process of fetching Indicators of Compromise (IoCs) from the MISP (Malware Information Sharing Platform & Threat Sharing) API, extracting relevant IoC information, and then updating QRadar reference sets with these IoCs using batch updates. The script also allows you to save the IoCs to a CSV file for further analysis and schedule the script to run periodically on Windows or Linux systems. 

## Features - **Fetch IoCs from MISP:** Retrieve IoCs by filtering on event IDs or attribute types (e.g., IPs, domains, file hashes). 
